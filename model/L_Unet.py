...
from model.layer_utils import crop_and_concat_L
# from model.layer_utils import pixel_wise_softmax


def Build_L_Unet(is_training, inputs, params):
    ...
    logging.critical("This is with Laplacian feature!  щ(ﾟДﾟщ)")


    output = inputs['images']
    laplacian = inputs ['laplacian']
    ...
    for layer in range(params.num_layers):
        ...
        with tf.variable_scope("Down_conv_{}".format(layer + 1)):          
            ...
            la_pools[layer] = laplacian
            if layer < params.num_layers - 1:
                output = maxpool(output, 2,2,2,2, "maxpool")
                laplacian = maxpool(laplacian, 2,2,2,2, 'la_pool')
    ...
    for layer in range(params.num_layers - 2, -1, -1):
        ...
        with tf.variable_scope("Up_deconv_{}".format(layer + 1)):
            ...
            output = crop_and_concat_L(la_pools[layer], output)
...
def _get_cost_L(logits, masks, params, class_weights, _variables):
...
def _get_optimizer_L(loss, params):
